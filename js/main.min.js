...
(function(){
	...
	/*实现quotes引用的隐藏，随机显示其一*/
	var Quotes = {};
	/*随机函数，参数为对象，
	 * 内置randomQuote方法
	 * 		该方法返回随机数
	 * 			（int转floor类型）
	 * 			随机数不能重复，次数等于对象长度时再次初始化
	 * 返回randomQuote方法
	 */
	Quotes.random = function(quotes){
		...
		var randomQuote = function(){
			...
			while(1){
				randomQuoteIndex = Math.floor(Math.random()*quoteCount);
	...
	/*
	 * 执行CSS属性集的自定义动画：将元素从一个状态改变为另一个状态
	 * 参数为对象，变化速度(变化间隔时间)
	 * 内置fader方法
	 * 		参数为淡出的元素，淡入的元素
	 * 		淡出，淡入
	...
	Quotes.animate = function(container,animSpeed){
		...
		var swap = function(){
			...
			activeQuoteElem=$(quotes[activeQuoteIndex]);
	...
	Quotes.init = function(){
		...
		var hiddentemplate = $(template).hide();

		container.html(hiddentemplate);

		Quotes.animate(container,25000);
	}
	
	/*定义扩展函数quote（记得return），
	 * 参数为对象数组，为每个对象调用Jquery构建方法，
	 * 返回对象
	 */
	$.fn.quote = function(){
		return Quotes.init.call(this);
	}
	...
	function Apptabs(){
		var apptabs = $('.apptab');
		var jsAppList = $('.js-app-list');
		var appTabDiv = $('.apptab-div');
		var prevList = jsAppList.eq(0);
		var activeList ;
		var selectionBar = $('.selectionBar');
		/*selectionBar为红色下划线块，
		 */
		apptabs.change(function(){
			if (apptabs.eq(0).is(':checked')) {
				activeList = jsAppList.eq(0);
				swiapplist(prevList,activeList);
				selectionBar.css('transform','translate(0%,0px) scaleX(0.333333)');
			}else if(apptabs.eq(1).is(':checked')) {
				activeList = jsAppList.eq(1);
				swiapplist(prevList,activeList);
				selectionBar.css('transform','translate(33.3333%,0px) scaleX(0.333333)');
			}else {
				activeList = jsAppList.eq(2);
				swiapplist(prevList,activeList);
				selectionBar.css('transform','translate(66.6666%,0px) scaleX(0.333333)');
			}
			prevList = activeList;
		});
		
		appTabDiv.mouseenter(function(){
			if (apptabs.eq(0).is(':checked')) {
				activeList = jsAppList.eq(0);
				swiapplist(prevList,activeList);
			}else if(apptabs.eq(1).is(':checked')) {
		...
		var swiapplist = function(fadeOut,fadeIn){
			fadeOut.fadeOut(0);
			fadeIn.fadeIn(0);
		}
	}
